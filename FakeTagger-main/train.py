...
TRUMP_PATH = 'resized_data/trump'
CAGE_PATH = 'resized_data/cage'
    ...
    with torch.no_grad() :
        for (trump_val_x, cage_val_x) in zip(trump_val_loader, cage_val_loader) :
            ...
            for k in range(0, 1, 1) :
                cv2.imwrite('result/trump/' + str(k) + '/trump_real_' + str(i) + '.png', (trump_val_x[k] * 255).permute(1, 2, 0).detach().cpu().numpy())
                cv2.imwrite('result/cage/' + str(k) + '/cage_real_' + str(i) + '.png', (cage_val_x[k] * 255).permute(1, 2, 0).detach().cpu().numpy())
            ...
            for k in range(0, 1, 1) :
                cv2.imwrite('result/trump/' + str(k) + '/trump_encoded_' + str(i) + '.png', (encoded_trump[k] * 255).permute(1, 2, 0).detach().cpu().numpy())
                cv2.imwrite('result/cage/' + str(k) + '/cage_encoded_' + str(i) + '.png', (encoded_cage[k] * 255).permute(1, 2, 0).detach().cpu().numpy())
            ...
            if i >= args.start_decode :
                ...
                for k in range(0, 1, 1) :
                    cv2.imwrite('result/trump/' + str(k) + '/trump_encoded_fake_' + str(i) + '.png', (encoded_trump_df[k] * 255).permute(1, 2, 0).detach().cpu().numpy())
                    cv2.imwrite('result/cage/' + str(k) + '/cage_encoded_fake_' + str(i) + '.png', (encoded_cage_df[k] * 255).permute(1, 2, 0).detach().cpu().numpy())
